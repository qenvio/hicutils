...
#' @param pos A vector with the genomic position IDs to be included in the output. It can be the \code{} column of the output from \code{\link{make_bins}}
#' @param whole Logical indicating if the entire available genome should be retrieved (rectangular contact map) or only the region of interest indicated with \code{region} (square contact map). It should be consistent tih \code{pos} argument
...
get_contact_matrix <- function(inbam, resolution, region, pos, whole = F, filtin = 0, filtex = 783){

    if(whole){
        
        script_file <- system.file("src", "bam_to_mat_whole.sh", package = "hicutils")
        #script_file <- "/home/evidal/mypackages/hicutils/inst/src/bam_to_mat_whole.sh"

    }else{

        script_file <- system.file("src", "bam_to_mat.sh", package = "hicutils")
        #script_file <- "/home/evidal/mypackages/hicutils/inst/src/bam_to_mat.sh"
